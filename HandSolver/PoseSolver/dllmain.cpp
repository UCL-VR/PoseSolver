...
EXPORT void setPoseConstant(hs::Pose3d* pose, bool constant)
{
    scene->problem.AddParameterBlock(pose->parameterBlock(), hs::Pose3d::Dimension);
    scene->problem.SetParameterBlockConstant(pose->parameterBlock());
}

EXPORT void setPoseVariable(hs::Pose3d* pose, bool constant)
{
    scene->problem.AddParameterBlock(pose->parameterBlock(), hs::Pose3d::Dimension);
    scene->problem.SetParameterBlockVariable(pose->parameterBlock());
}
...
EXPORT hs::PreIntegrationFactor* addPreIntegrationFactor(hs::Pose3d* start, hs::Pose3d* end)
{
    auto factor = new hs::PreIntegrationFactor(start, end);
...
/// second.
/// </summary>
EXPORT void addImuMeasurement(hs::PreIntegrationFactor* factor, float ax, float ay, float az, float gx, float gy, float gz, float dt)
{
    factor->addSample(Eigen::Vector3d(ax, ay, az), Eigen::Vector3d(gx, gy, gz), dt);
}

EXPORT hs::ImuOrientationFactor* addImuOrientationFactor(hs::Pose3d* reference, hs::Pose3d* start)
{
    auto factor = new hs::ImuOrientationFactor(reference, start);
    factor->addToProblem(scene->problem);
    return factor;
}

EXPORT void addImuOrientationMeasurement(hs::ImuOrientationFactor* factor, float ax, float ay, float az, float gx, float gy, float gz, float dt)
{
    factor->addSample(Eigen::Vector3d(ax, ay, az), Eigen::Vector3d(gx, gy, gz), dt);
}
...
/// </summary>
EXPORT hs::ImuBiasParameters* addImuBiasParameters()
{
    auto parameterBlock = new hs::ImuBiasParameters();
...
/// </summary>
/// <returns></returns>
EXPORT hs::ImuBiasFactor* addImuBiasFactor(hs::ImuBiasParameters* biasParameters, Unity::MotionFrame referenceFrame, Unity::Vector3 accelerometer, Unity::Vector3 gyroscope)
{
    return new hs::ImuBiasFactor(scene->problem, biasParameters, referenceFrame, accelerometer, gyroscope);
}

EXPORT void removeImuBiasFactor(hs::ImuBiasFactor* factor)
{
    scene->problem.RemoveResidualBlock(factor->id);
}

EXPORT Unity::ImuBiasParameters getImuBiasParameters(hs::ImuBiasParameters biasParameters)
{
