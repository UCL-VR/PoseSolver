...
TEST(DenavitHartenberg, Single) {
	...
	auto x1 = new Transformd();
	auto x2 = new Transformd(Vector3d::UnitZ());
...
TEST(DenavitHartenberg, StartFromZero) {
	...
	auto x1 = new Transformd();
	auto x2 = new Transformd(AngleAxisd(2.34, Vector3d::UnitY()) * Vector3d::UnitZ(), AngleAxisd(2.34, Vector3d::UnitY()));
	auto joint = new dh::Joint();
...
TEST(DenavitHartenberg, OffsetPointObservation) {
	...
	auto expected_x2_pose = Transformd(AngleAxisd(1.7, Vector3d::UnitY())) * Transformd(Vector3d::UnitZ());
...
TEST(DenavitHartenberg, MultiChainOnly) {
	...
	auto m1 = new observations::PointMeasurement();
	m1->pose = x1;

	auto m2 = new observations::PointMeasurement();
	m2->pose = x3;
...
TEST(DenavitHartenberg, MultiChainMultiPointObservation) {
	...
	auto expected_x3_pose = new Transformd(Vector3d(0.5, 0, 0.9), AngleAxisd(0.8, Vector3d::UnitY()));

	auto mx31 = new observations::PointMeasurement();
	mx31->pose = x3;
	mx31->offset = Vector3d(0.1, 0, 0.2);
	mx31->point = (*expected_x3_pose) * mx31->offset;

	auto mx32 = new observations::PointMeasurement();
	mx32->pose = x3;
...
TEST(DenavitHartenberg, JointLimit) {
	...
	joint->start = new Transformd();
	joint->end = new Transformd();
	joint->theta = 0.0;
	...
	auto measurement = new observations::PointMeasurement();
