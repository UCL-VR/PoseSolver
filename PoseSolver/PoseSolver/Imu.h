...
namespace imu {
	...
	/// imu measurements and a calibration transforms.
	/// </summary>
	...
	public:
		...
		PreIntegrationFactor(Transformd* begin, Transformd* end) {
			this->start = begin;
			this->end = end;
		...
		// parameter blocks.
		// The residual is end (i.e. a Transformd)
		...
		struct CostFunctor1 {
			...
			bool operator()(
				...
				auto start = transforms::Transform<T>::Map(before);

				// Apply the transforms to the body
				transforms::Transform<T> end = start;
				...
				// the transforms at end to get the residuals.
				transforms::Transform<T>::Between(end, transforms::Transform<T>::Map(after), residuals);
...
#pragma pack(pop)
	...
	public:
		...
		ImuBiasFactor(ceres::Problem& problem, ImuBiasParameters* calibration, motion::MotionFrame reference, Eigen::Vector3d a, Eigen::Vector3d d)
		{
		...
		struct CostFunctor
		{
			...
			bool operator()(
				...
				auto localRotation = transforms::Rodrigues<T>(
	...
	public:
		...
		ImuCalibrationFactor(ceres::Problem& problem, ImuCalibrationParameters* calibration, motion::MotionFrame reference, Eigen::Vector3d a, Eigen::Vector3d d)
		{
		...
		struct CostFunctor 
		{
			...
			bool operator()(
				...
				// The estimated transforms (the local offset composed with the known strapdown frame)
				auto s = factor->reference.pose.Cast<T>() * transforms::Transform<T>::Map(local);
	...
	public:
		...
		/// </summary>
		transforms::Transformd* reference;

		/// <summary>
		/// The absolute transform of the IMU estimated in world space. This
		/// is the equivalent of the start transforms in the pre-integration factor.
		/// The local transform is given by the difference between reference
		/// and this.
		/// </summary>
		transforms::Transformd* start;

		Eigen::Vector3d acceleration;

		ImuOrientationFactor(transforms::Transformd* reference, transforms::Transformd* start)
		{
		...
		struct CostFunctor1 {
			...
			bool operator()(
				...
				// The estimated transforms`
				auto s = transforms::Transform<T>::Map(start);
				...
				// The position parameters should match the reference frame exactly
				Vector3<T>::Map(&residuals[3]) = transforms::Transform<T>::Map(reference).Position() - s.Position();
